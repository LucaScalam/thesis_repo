...
#         X_data = eop.data['BANDS']
        ### Just true color and NIR bands
        X_data = eop.data['BANDS'][:,:,:,[1,2,3,7]]
        ### Raw vector -> [np.newaxis, ...]
...
def save_into_chunks(config: CreateNpzConfig, npys_dict: Dict[str, np.ndarray]) -> None:
    ...
    for idx, i in enumerate(range(0, len(npys_dict['X']), chunk_size)):
        ...
        np.savez(open(os.path.join(config.output_folder, f'{filename}.npz'), 'wb'),
    ...
    pd.concat(dfs).to_csv(open(config.output_dataframe, 'w'), index=False)

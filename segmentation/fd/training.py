...
#     filesystem = prepare_filesystem(config)
    ...
    data = np.load(open(os.path.join(config.npz_folder, chunk),'rb'), allow_pickle=True)
#     print(data)
...
#                 print(data[key][idx_fold])

            fold_folder = os.path.join(config.npz_folder, f'fold_{fold}')

#             os.makedir(fold_folder, recreate=True)
#             print(os.path.exists(fold_folder))
            if not os.path.exists(fold_folder):
                os.mkdir(fold_folder)

            np.savez(open(os.path.join(fold_folder, chunk), 'wb'), **patchlets)

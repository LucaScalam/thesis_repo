...
class SamplePatchlets(EOTask):
    ...
    a certain percentage of valid pixels. Here, we denormalized the data.
    """
...
#                 print(np.sum(patchlet != self.no_data_value))
                ratio = np.sum(patchlet != self.no_data_value) / (self.patch_size * self.patch_size)
#                 print(f'index: {retry_count}, ratio: {ratio}')
                retry_count += 1
#             print('-------------------------')
            if retry_count == self.max_retries:
                LOGGER.warning(f'Could not determine an area with good enough ratio of valid sampled pixels (= enough ground truth data) for '
...
#                         sampled_data = feature_data[:, row:row + self.patch_size, col:col + self.patch_size, :]
                        ### Denormalized data
                        sampled_data = feature_data[:, row:row + self.patch_size, col:col + self.patch_size, :]*10000
